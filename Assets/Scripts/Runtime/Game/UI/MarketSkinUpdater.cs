...
public class MarketSkinUpdater : MonoBehaviour
{
    ...
    private void Construct(ISettingProvider settingProvider, UserInventoryService userInventoryService)
    {
        ...
        bool purchased = userInventoryService.GetInventory().PurchasedMarketArea;
        ...
        if (purchased)
        {
            for (int i = 0; i < config.Areas.Count; i++)
            {
                ...
                if (area.Type == AreaType.Market && area.ID != 0)
                {
